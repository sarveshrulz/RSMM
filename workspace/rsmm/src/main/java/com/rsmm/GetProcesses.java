...
@ServerEndpoint("/processes")
public class GetProcesses {
	Process process;

	@OnMessage
	public void getProcesses(Session session, String msg, boolean last) throws InterruptedException {
		Login login = new Login();

		String command = "sshpass -f ~/.sshpass ssh -t -t " + login.getHostname() + " -p " + login.getPort()
				+ " /home/sarvesh/.local/bin/getProcesses";
		if (session.isOpen()) {
